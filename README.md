...
| PROC-*      | 2/4   | PARTIAL (M003 in progress). PROC-COCHANGE-003 + PROC-LINECOUNT-004 ACTIVE 2026-06-22. PROC-SETUP-001 + PROC-UPDATE-002 still planned. |
| TOOL-*      | 0/2   | Pending M004. 2 IDs registered in STD-META-001 §4.15 but files not yet created.              |
| instructions/ | 4   | CREATED 2026-06-22. 4 PROC-*.md placeholder+spec files.                                         |
| scripts/    | 3     | CREATED 2026-06-22. `build-registry.py`, `co-change-check.sh`, `line-count-check.sh`.            |
| tools/      | 0     | Not started. Planned as TOOL-* scripts.                                                       |
| registry.json | 1   | CREATED 2026-06-22. Auto-generated by `scripts/build-registry.py` from INDEX.md + META-001-id-registry.md. 27 IDs (17 RULE + 4 PROC + 6 TOOL). |
| MIGRATIONS.md | 0   | Not started. Migrations currently tracked in `Z-ai-standards/MIGRATIONS.md`.                  |

**Net enforcement coverage**: rules are *declared* and 2 of 4 planned
procedures now exist with working scripts. Pre-commit + CI continue to
run the L1 verifiers (`verify-standards.js`, `verify-id-graph.js`);
the 2 new L2 procedures (`PROC-COCHANGE-003`, `PROC-LINECOUNT-004`)
can be wired as pre-commit hooks via `install-hooks.sh` (see
§"Integration" in each `instructions/PROC-*.md`). Full runtime
enforcement still pending M003 completion (PROC-SETUP/UPDATE) and M004
(TOOL-VERIFY-001, TOOL-BUMP-005).
